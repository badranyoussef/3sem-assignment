...
public class HotelDAONotCorrectextend extends DAO<Hotel, Integer> {
    ...
    public List getAll() {
        try (var em = emf.createEntityManager()) {
            ...
            List<HotelDto> dtos = hotels.stream().map(hotel -> HotelDto.builder()
                    .id(hotel.getId())
                    .name(hotel.getName())
                    .address(hotel.getAddress())
                    .build()
            ).toList();
            //new HotelDto(hotel.getId(), hotel.getName(), hotel.getAddress())).collect(Collectors.toList());
            return dtos;
        }
    }
